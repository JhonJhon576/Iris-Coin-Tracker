...
export default {
    ...
    dataGridEditFormatter(obj) {
        ...
        data_sourcesManyListFormatter(val) {
            if (!val || !val.length) return []
            return val.map((item) => item.url)
        },
        data_sourcesOneListFormatter(val) {
            if (!val) return ''
            return val.url
        },
        data_sourcesManyListFormatterEdit(val) {
            if (!val || !val.length) return []
            return val.map((item) => {
              return {id: item.id, label: item.url}
            });
        },
        data_sourcesOneListFormatterEdit(val) {
            if (!val) return ''
        ...
        potsManyListFormatter(val) {
            if (!val || !val.length) return []
            return val.map((item) => item.name)
        },
        potsOneListFormatter(val) {
            if (!val) return ''
            return val.name
        },
        potsManyListFormatterEdit(val) {
            if (!val || !val.length) return []
            return val.map((item) => {
              return {id: item.id, label: item.name}
            });
        },
        potsOneListFormatterEdit(val) {
            if (!val) return ''
            return {label: val.name, id: val.id}
        },
